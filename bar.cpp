...
namespace game {
	...
	GameObject* Bar::GetChild(int i) {
		if (i >= chile_game_objects_.size()) { return NULL;}
		else {
			return chile_game_objects_[i];
		}
	}
	...
	void Bar:: RenderMap(glm::mat4 view_matrix, double current_time) {
		if (ghost) { return; }
