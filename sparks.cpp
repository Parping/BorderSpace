...
namespace game {
    ...
    float rand_num2(void)
    {
        return ((float)rand()) / ((float)RAND_MAX);
    }

    void Sparks::CreateGeometry(int num_particles)
    {
...
// Vertices
        ...
        GLfloat* Sparks = new GLfloat[num_particles * vertex_attr];
        float theta, r, tmod;
        ...
        for (int i = 0; i < num_particles; i++) {
            ...
            if (i % 4 == 0) {
                ...
                //
                // Opening of the stream of Sparks
                theta = rand_num2() * glm::pi<float>();
            ...
            Sparks[i * vertex_attr + 0] = vertex[(i % 4) * 7 + 0];
            Sparks[i * vertex_attr + 1] = vertex[(i % 4) * 7 + 1];

            // Set direction (velocity) based on random values
            Sparks[i * vertex_attr + 2] = sin(theta);
            Sparks[i * vertex_attr + 3] = cos(theta);

            // Set phase based on random values
            Sparks[i * vertex_attr + 4] = tmod;

            // Copy texture coordinates from standard sprite
            Sparks[i * vertex_attr + 5] = vertex[(i % 4) * 7 + 5];
            Sparks[i * vertex_attr + 6] = vertex[(i % 4) * 7 + 6];
        }
        ...
        glBufferData(GL_ARRAY_BUFFER, num_particles * vertex_attr * sizeof(GLfloat), Sparks, GL_STATIC_DRAW);
    ...
    void Sparks::SetGeometry(GLuint shader_program) {
